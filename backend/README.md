...
Toàn bộ lệnh Python trong tài liệu này được chạy từ thư mục `backend/`:
...
`walkway-server` chỉ cung cấp REST API dưới `/api` và WebSocket dưới `/ws`.
FastAPI không mount hoặc phục vụ file build của frontend; frontend phải chạy bằng
service hoặc container riêng và proxy các route này sang backend khi cần cùng origin.
